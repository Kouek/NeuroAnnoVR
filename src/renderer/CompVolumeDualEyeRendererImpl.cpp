#include "CompVolumeDualEyeRendererImpl.h"

#include <Common/transfer_function_impl.hpp>

std::unique_ptr<kouek::CompVolumeDualEyeRenderer>
kouek::CompVolumeDualEyeRenderer::create(const CUDAParameter& cudaParam)
{
	return std::make_unique<CompVolumeDualEyeRendererImpl>(cudaParam);
}

kouek::CompVolumeDualEyeRendererImpl::CompVolumeDualEyeRendererImpl(
	const CUDAParameter& cudaParam)
	: cuda(cudaParam)
{
	CompVolumeDualEyeRendererImplCUDA::CUDAParameter param;
	param.texUnitDim = cuda.texUnitDim;
	CompVolumeDualEyeRendererImplCUDA::uploadCUDAParameter(&param);
}
...
void kouek::CompVolumeDualEyeRendererImpl::registerGLResource(
	GLuint outColorTex, GLuint inDepthTex, uint32_t w, uint32_t h)
{
	renderParam.windowSize = { w,h };
	CompVolumeDualEyeRendererImplCUDA::registerGLResource(outColorTex, inDepthTex,
		w, h);
}

void kouek::CompVolumeDualEyeRendererImpl::unregisterGLResource()
{
	CompVolumeDualEyeRendererImplCUDA::unregisterGLResource();
}

void kouek::CompVolumeDualEyeRendererImpl::setStep(uint32_t maxStepNum, float step)
{
	renderParam.maxStepNum = maxStepNum;
	renderParam.step = step;
}

void kouek::CompVolumeDualEyeRendererImpl::setSubregion(const Subregion& subrgn)
{
	renderParam.subrgn = subrgn;
	subrgnChanged = true;
}

void kouek::CompVolumeDualEyeRendererImpl::setTransferFunc(const vs::TransferFunc& tf)
{
	vs::TransferFuncImpl tfImpl(tf);
	CompVolumeDualEyeRendererImplCUDA::uploadTransferFunc(
		tfImpl.getTransferFunction().data());
	CompVolumeDualEyeRendererImplCUDA::uploadPreIntTransferFunc(
		tfImpl.getPreIntTransferFunc().data());
}

void kouek::CompVolumeDualEyeRendererImpl::setLightParam(const LightParamter& lightParam)
{
	renderParam.lightParam = lightParam;
}

void kouek::CompVolumeDualEyeRendererImpl::setVolume(
	std::shared_ptr<vs::CompVolume> volume)
{
...

	{
		...
		// upload to CUDA
		CompVolumeDualEyeRendererImplCUDA::uploadBlockOffs(
...

	{
		...
		CompVolumeDualEyeRendererImplCUDA::uploadCompVolumeParam(&compVolumeParam);
	}

	{
		auto& texObj = blockCache->GetCUDATextureObjects();
		CompVolumeDualEyeRendererImplCUDA::uploadCUDATextureObj(
...
void kouek::CompVolumeDualEyeRendererImpl::render()
{
	kouek::CompVolumeDualEyeRendererImplCUDA::uploadRenderParam(&renderParam);
	...
	CompVolumeDualEyeRendererImplCUDA::uploadMappingTable(
		mappingTable.data(), sizeof(uint32_t) * mappingTable.size());

	CompVolumeDualEyeRendererImplCUDA::render(
		renderParam.windowSize.x, renderParam.windowSize.y);
}

void kouek::CompVolumeDualEyeRendererImpl::setCamera(const CameraParameter& camParam)
{
